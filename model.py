...
class Model:
    ...
    def analyze_threshold(self, X, Y):
        ...
        #print(threshold)
        writer = open('stat_threshold.csv', 'a')
        writer.write(str(threshold).strip('[] ') + '\n')
        writer.close()
        #print(self.models[0].feature_importances_)
        writer = open('stat_features.csv', 'a')
        writer.write(','.join(map(str, self.models[0].feature_importances_)) + '\n')
        writer.close()
        pass
